
import React, { useState } from 'react';
import { ArrowLeftIcon, EyeIcon, EyeOffIcon } from './Icons';

interface LoginPageProps {
  onLoginAttempt: (username: string, password: string) => boolean;
  onBack: () => void;
  onDevLogin: () => void;
}

const LoginPage: React.FC<LoginPageProps> = ({ onLoginAttempt, onBack, onDevLogin }) => {
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [showPassword, setShowPassword] = useState(false);

  const handleLogin = () => {
    const success = onLoginAttempt(username, password);
    if (!success) {
      setError('Credentials Invalid.');
    }
  };

  return (
    <div className="h-screen flex items-center justify-center w-full p-4 relative">
      <div className="w-full max-w-sm p-8 md:p-10 border border-gray-200 bg-white shadow-2xl shadow-gray-200/50">
        <button onClick={onBack} className="flex items-center gap-2 text-gray-500 mb-8 text-sm hover:text-[#1E90FF] transition-colors">
          <ArrowLeftIcon className="h-4 w-4" /> Return
        </button>
        
        <h2 className="text-3xl font-serif">Access Terminal</h2>
        <p className="text-gray-500 text-sm mb-8">Please authenticate.</p>
        
        <div className="space-y-6">
          <div>
            <label className="block text-xs text-gray-500 mb-2 uppercase tracking-wider">Identifier</label>
            <input 
              type="text" 
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              placeholder="Username" 
              className="w-full bg-transparent border-b border-gray-300 py-2 text-black placeholder-gray-400 focus:border-[#1E90FF] focus:outline-none transition"
            />
          </div>
          
          <div>
            <label className="block text-xs text-gray-500 mb-2 uppercase tracking-wider">Passcode</label>
            <div className="relative">
              <input 
                type={showPassword ? 'text' : 'password'}
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                placeholder="••••••" 
                className="w-full bg-transparent border-b border-gray-300 py-2 text-black placeholder-gray-400 focus:border-[#1E90FF] focus:outline-none transition"
              />
              <button onClick={() => setShowPassword(!showPassword)} className="absolute right-0 top-1/2 -translate-y-1/2 p-2 text-gray-400 hover:text-[#1E90FF]">
                {showPassword ? <EyeOffIcon className="h-5 w-5" /> : <EyeIcon className="h-5 w-5" />}
              </button>
            </div>
          </div>

          <button 
            onClick={handleLogin} 
            className="w-full bg-[#1E90FF] text-white py-3 rounded-md font-semibold transition-all hover:bg-[#1079e0]"
          >
            Enter System
          </button>
          {error && <p className="text-red-500 text-sm text-center">{error}</p>}
        </div>
      </div>
      <button 
        onClick={onDevLogin}
        className="absolute bottom-4 right-4 bg-gray-100 text-gray-500 text-xs px-3 py-1 rounded-md border border-gray-200 hover:bg-gray-200 hover:text-gray-700 transition-colors"
      >
        Dev Login
      </button>
    </div>
  );
};

export default LoginPage;